...
> **ver. 1.1** ｜ **Last edited: 2026-04-15**
...
**情境 B（檔案存在但沒標記）**：
1. `Read` 出既有內容（我們在 Step 1 已經讀過了）
2. 在對話中把「既有內容 + 兩個空行 + AI 分身起始助手紀錄區塊」拼成新字串
3. 用 `Write` 寫回根目錄的 `CLAUDE.md`
4. **寫入前一定要告訴用戶**：「我偵測到你已經有 `CLAUDE.md` 了，我會保留你的原內容，把AI 分身起始助手紀錄加在檔案最後面。你原本的內容不會被改動。」
