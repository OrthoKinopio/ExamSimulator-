...
| 版本         | 長什麼樣                    | 適合誰                | 建議                    |
| :--------- | :---------------------- | :----------------- | :-------------------- |
| **終端機版**   | 黑底白字的文字介面               | 想要完整功能、願意學一點點終端機操作 | ✅ 雷蒙主力用               |
| **桌面 App** | 有視窗介面、可以拖檔案進去           | 覺得終端機太有壓力、想要圖形介面   | ✅ 對小白友善，推薦使用；雷蒙另一個帳號用 |
| **IDE 整合** | VS Code / Cursor 裡面的側邊欄 | 已經在用這些編輯器的人        | 如果你不知道 IDE 是什麼，建議跳過   |

**終端機版和桌面 App 都推薦。** 這堂課主要以終端機和桌面版做示範，但桌面 App 對新手來說也很好用，有圖形介面、可以直接拖檔案進去、操作更直覺；兩個可以並行使用，看個人偏好哪個順手就用哪個。
